...
**Corrected against the LIVE DB 2026-06-14 (an earlier draft wrongly said "RLS is already live on these tables").** Actual live posture as of that date:
...
- RLS **OFF**: `classes`, `schools` — policies exist but were **dormant/unenforced** (`disable_all_rls` never re-enabled them); `authenticated` held a plain SELECT grant, so every signed-in user could read every class/school row.

**Re-corrected against the LIVE DB 2026-08-06** (the irony of a "corrected against live" block itself going stale is worth noting): RLS is now **ON** with real policies on all six schools org tables — `classes`(6 policies), `schools`(6), `groups`(1), `govt_admins`(4), `invite_codes`(8), `entitlement_grants`(1) — verified directly via `pg_class.relrowsecurity` + `pg_policies`. The "first paying school" tightening pass this section originally deferred has since happened; **do not read the "DO NOT enable RLS" instruction below as still live** — it described the 2026-06-14 state, not today's. Any *further* changes to these tables' policies still go through CLAUDE.md's canary doctrine as normal DB-auth changes; that discipline hasn't changed, only the starting posture has.
...
- `classes_select/insert/update` — **RLS is now ON here (verified 2026-08-06; was dormant when this line was first written 2026-06-14) — do not assume dormancy, check `pg_policies` before touching.** Note `classes_select`'s membership branch is role-LESS (matches *students* too); `is_class_teacher` is role-filtered — do **not** copy that role-less `tag_type='class'` shape onto `user_tags`/`class_sessions` or you'd leak classmates' rows to students.
...
2. **RLS rebase** (its own migration) — §5 owner→membership via `is_class_teacher` on the 3 live sites (incl. `can_view_learner_data`), god + school-admin branches preserved. **`classes`/`schools` RLS is already enabled (verified 2026-08-06 — CLAUDE.md's first-paying-school pass has since happened)**, so this step now also needs those tables' own policies checked against the membership model, not treated as out-of-scope-because-dormant.
