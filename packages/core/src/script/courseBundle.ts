 ...
 */
export type BundlePhraseRole = 'build' | 'use'
...
// ============================================================================
// SCRIPT ARTIFACT IDENTITY
// ============================================================================

/**
 * Per-round script shape — phrase counts + Fibonacci spaced-rep schedule.
 * Structurally identical to `ScriptShapeConfig` in
 * `player-vue/src/composables/useAlgorithmConfig.ts` (the admin-tweakable
 * `algorithm_config` key='script_shape' row) — duplicated here rather than
 * imported so `@ssi/core` stays framework-free. Assignable either direction
 * (same field names/types); keep the two in lockstep by hand until the
 * cutover (design §5 step 6) makes the bundle the only reader of shape data.
 */
export interface BundleScriptShape {
...
export interface CourseBundle {
  ...
  /**
   * Identical value to `version` — the artifact-identity name (design §2).
   * `version` stays for backward compat during the transition and retires
   * with `courses.version`/round-map.ts (§5 step 7); new code should read
   * `contentVersion`.
   */
  contentVersion: string | number
  /**
   * Resolved `algorithm_config` key='script_shape' values, embedded so the
   * client generates from what it was actually issued instead of a separate
   * `algorithm_config` read (design §2, §3).
   */
  scriptShape: BundleScriptShape
  /** `algorithm_config.version` for the script_shape row at fetch time — the
   *  artifact identity's shape-version component (design §2). */
  scriptShapeVersion: number
  /**
   * The shared generator's `GENERATOR_VERSION` constant
   * (`@ssi/core`'s `script/generateScript.ts`), echoed by the server so a
   * stored/replayed artifact id is self-describing from the bundle alone.
   */
...
/** O(1) lookup by seedId — feeds SEED-PHASE spaced-rep review generation. */
export function seedsBySeedId(bundle: CourseBundle): Map<string, BundleSeed> {
  const map = new Map<string, BundleSeed>()
  for (const seed of bundle.seeds) {
    map.set(seed.seedId, seed)
  }
  return map
}
