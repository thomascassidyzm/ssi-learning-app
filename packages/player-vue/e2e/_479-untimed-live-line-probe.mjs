// Job #479: the #468 probe, plus the PAINT of each line — computed colour,
// background-image and background-clip of every .breath-fill per frame — so the
// assertion is 'the untimed live line is painted whole in --text-primary, no
// gradient, no clip; said lines are the said colour; ahead lines the dim'.
// Job #468: the #430 probe with the #428 pod-cards step — opens the pod card named by GROUP first.
...
const trackerState = () => page.evaluate(() => {
  ...
  return {
    ...
    groups: groups.map((g) => ({
      ...
      paint: (() => { const f = g.querySelector('.breath-fill'); const cs = f ? getComputedStyle(f) : null; return cs ? { color: cs.color, bgImage: cs.backgroundImage === 'none' ? 'none' : 'gradient', clip: cs.webkitBackgroundClip || cs.backgroundClip, groupColor: getComputedStyle(g).color } : null })(),
...
if (out.modeTriggerFound) {
  ...
  // Pod cards (job #428): the list opens on one card per pod; tap the one
  // named by GROUP, then the scene cards of that pod are the siblings below.
  const podCard = page.locator('.pod-card').filter({ hasText: GROUP || /./ }).first()
  out.podCards = await page.locator('.pod-card').allInnerTexts().catch(() => [])
  if (await podCard.count()) {
    await podCard.click({ timeout: 5000 }).catch((e) => (out.podCardError = String(e).slice(0, 150)))
    await page.locator('.scene-card:not(.pod-card)').first().waitFor({ state: 'visible', timeout: 60000 }).catch((e) => (out.podOpenWaitError = String(e).slice(0, 120)))
    await page.waitForTimeout(800)
    await page.screenshot({ path: `${OUT_DIR}/${TAG}-scenes.png` }).catch(() => {})
  }
