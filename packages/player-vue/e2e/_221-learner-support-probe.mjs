// Job #221 live probe — a learner's own reply thread in the platform Support
// inbox on staging, as an ssi_admin meets it. It LISTS (with "show answered"
// on, since this thread is already answered from #821's own test), OPENS the
// existing test learner's thread, and SENDS one more reply, then reads the
// row back to prove it was authored as the admin.
// The thread belongs to thomas.cassidy+colombo-wall@gmail.com — the same test
// learner account job #821 used to prove the reply path — no real learner is
// written to.
...
const ADMIN_EMAIL = process.env.EMAIL || 'thomas.cassidy+e2e-admin@gmail.com'
const THREAD = process.env.THREAD || '6028728c-8afb-49db-9189-1ccf35b650e0'
const OUT = process.env.OUT || '/home/tomcassidy/probe-221-learner-support'
...
const link = await fetch(`${U}/auth/v1/admin/generate_link`, {
  method: 'POST', headers: H, body: JSON.stringify({ type: 'magiclink', email: ADMIN_EMAIL, options: { redirect_to: `${BASE}/admin/support` } }),
}).then((r) => r.json())
...
console.log('signed in as', verified.user.id, ADMIN_EMAIL)
...
const originBefore = await rest(`support_threads?select=origin_message_id&id=eq.${THREAD}`)
const originId = originBefore[0]?.origin_message_id
const originMsgBefore = originId ? await rest(`user_messages?select=id,read_at&id=eq.${originId}`) : []
console.log('origin message read_at before:', originMsgBefore[0]?.read_at)
...
// This test thread is already answered (from job #821's own proof), so show answered too.
...
for (let i = 0; i < n; i++) {
  ...
  if (i < 20) console.log(`  [${i}] ${t.replace(/\n/g, ' | ').slice(0, 140)}`)
  if (learnerRow < 0 && /Colombo wall/.test(t) && /her own message/i.test(t)) learnerRow = i
}
if (learnerRow < 0) { console.error('the learner test thread is not in the list, named as a learner row'); await browser.close(); process.exit(1) }

await heads.nth(learnerRow).click()
...
const text = `Job #221 staging check, ${stamp} — answered from the platform inbox at Admin → Support.`
...
const originMsgAfter = originId ? await rest(`user_messages?select=id,read_at&id=eq.${originId}`) : []
console.log('origin message read_at after (should be null — unread again for her):', originMsgAfter[0]?.read_at)

const doorbellCheck = await rest(`support_messages?select=id,doorbell_sent_at&id=eq.${mine?.id}`)
console.log('doorbell_sent_at on the new row (should stay null — the doorbell skips a learner thread):', doorbellCheck[0]?.doorbell_sent_at)
