// Job #379 (from #354's probe, course parameterised via COURSE): headless proof that Italian Listening Mode lists the served pod
...
}, { ref: REF, sess: session, course: process.env.COURSE || 'ita_for_eng' })
...
page.on('response', async (r) => {
  const u = r.url()
  if (/listening_pod|\/bundle|course_audio|serving|learner_pod/.test(u)) {
    let len = -1
