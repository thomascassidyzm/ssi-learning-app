 ...
 *
 * The mode is "we are playing from cache because we could not reach new
 * content". Its whole safety property is that NO LEGO progress is captured
 * while it holds — so a weak connection cannot move a learner, by construction
 * rather than by a check happening to be right.
 *
 * The gate itself lives in LearningPlayer.vue as consolidatingBlocksProgressWrite
...
function makePlayer() {
  const writes: Write[] = []
  const state = { consolidating: false, cursorLegoId: 'S0013L02', ceilingLegoId: 'S0013L02' }
  let pendingCycle: number | null = null

  const blocked = () => state.consolidating
  ...
  return {
    ...
    leaveConsolidating() { state.consolidating = false },
...
describe('CONSOLIDATING mode', () => {
  let player: ReturnType<typeof makePlayer>
  beforeEach(() => { player = makePlayer() })

  it('records progress normally when it is NOT consolidating', () => {
