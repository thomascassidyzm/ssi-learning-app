...
describe('backendCyclesToRounds — A-64 floor on the instant-playback path', () => {
  ...
  it('holds across the round seam for ordinary cycles', () => {
    // Round 1 ends on two identical build phrases; round 2's practice cycles
    // repeat the same text. The seam must not produce a third in a row.
    const same = { known_text: 'same', target_text: 'pareil' }
    const r1 = [
      backendCycle({ type: 'intro', known_text: 'i1', target_text: 'i1-t' }),
      backendCycle({ type: 'build', ...same }),
      backendCycle({ type: 'build', ...same }),
    ]
    const r2 = [
      backendCycle({ type: 'build', ...same }),
      backendCycle({ type: 'build', known_text: 'other', target_text: 'autre' }),
    ]
    const rounds = backendCyclesToRounds(
      (legoId: string) => (legoId === 'L1' ? r1 : r2) as any,
      { rounds: [{ r: 1, legoId: 'L1', seed: 1 }, { r: 2, legoId: 'L2', seed: 2 }] } as any,
    )
    expect(findConsecutiveBreach(flatCycles(rounds), cyclePromptIdentity)).toBe(-1)
  })
...
describe('the round opening is anchored', () => {
  it('never pulls a build phrase in front of the intro, even at a colliding seam', () => {
    // Round 1 ends on two cycles identical to round 2's intro text. Without the
    // anchor, round 2's intro would be a third consecutive repeat and the cap
    // would hoist a build phrase in front of it, breaking round structure.
    const dupText = { known_text: 'same', target_text: 'pareil' }
    const r1 = [backendCycle({ type: 'build', ...dupText }), backendCycle({ type: 'build', ...dupText })]
    const r2 = [
      backendCycle({ type: 'intro', ...dupText }),
      backendCycle({ type: 'debut', ...dupText }),
      backendCycle({ type: 'build', known_text: 'b', target_text: 'b-t' }),
    ]
    const rounds = backendCyclesToRounds(
      (legoId: string) => (legoId === 'L1' ? r1 : r2) as any,
    ...
    const second = rounds.find(r => r.roundNumber === 2)!
    expect((second.cycles[0] as any).type).toBe('intro')
    expect((second.cycles[1] as any).type).toBe('debut')
  })
})
