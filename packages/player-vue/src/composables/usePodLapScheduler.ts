 ...
 *     pipeline. Played in early stages so the learner gets the chunks
 *     spelled out; falls out of the playlist by Stage 2+. If the
 *     sentence has no explainer_audio_id (intentional one-chunk skip
 *     per the methodology, or upstream gap) the slot is silently
 *     dropped — same convention as 'trans' on a sentence with no
 *     known_audio_id.
 */
...
export interface PodSentenceRow {
  ...
  /** Tom-voiced bilingual chunk-by-chunk explainer ("'X' means Y. 'A' means B.").
   *  Populated by Popty's pod-explainer batch; null when the sentence is a
   *  single-chunk intentional skip (one proper noun, etc.) or hasn't been
   *  rendered yet. Players treat null as "drop the 'explainer' slot from
   *  this sentence's playlist", same shape as 'trans' skipping when
   *  known_audio_id is null. */
