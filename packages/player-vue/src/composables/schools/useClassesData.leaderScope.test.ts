...
describe('useClassesData — a leader who also teaches', () => {
  ...
  it("surfaces the DB's OWN reason, not a flattened 'Failed to fetch classes'", async () => {
    // PostgREST errors are plain objects, not Error instances, so the generic
    // catch flattened them and the client had nothing useful to say. That is
    // why the empty Classes tab of 2026-08-07 took a live DB session to
    // diagnose. fetchClassDetail already dodged this trap; fetchClasses did
    // not. (A silent RLS filter still yields no error at all — that is the
    // policy layer, fixed in 20260807c/d — but a GRANT-layer denial says
    // "permission denied" and must reach the screen.)
    taught.ids = []
    const cd = await setup({
