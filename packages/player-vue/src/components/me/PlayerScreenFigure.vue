 ...
 */
const CALLOUTS = [
  { x: 54, y: 27.9, text: t('figure.calloutOneGo', 'One go: you hear it, then your turn, then twice properly.') },
  { x: 81, y: 83, text: t('figure.calloutOffline', 'Offline downloads live here.') },
  { x: 11, y: 90.5, text: t('figure.calloutCourses', 'Every course you have.') },
  { x: 43.5, y: 90, text: t('figure.calloutPlay', 'The only button that matters.') },
]
...
<template>
  <div class="psf">
    <button type="button" class="psf-thumb" @click="open = true">
      ...
      <span class="psf-thumb-label">{{ t('figure.showMeTheScreen', 'Show me the screen') }}</span>
    </button>

    <Teleport to="body">
      <div v-if="open" class="psf-sheet" role="dialog" aria-modal="true" :aria-label="t('figure.thePlayerScreen', 'The player screen')">
        <header class="psf-header">
          <span class="psf-title">{{ t('figure.thePlayerScreen', 'The player screen') }}</span>
          <button type="button" class="psf-close" :aria-label="t('common.close', 'Close')" @click="open = false">
