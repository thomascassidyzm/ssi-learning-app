...
<template>
  <div class="lego-assembly" :lang="targetLang" :class="[assemblyPhase, { 'instant-hide': instantHide }]" :style="{ '--sentence-scale': sentenceScale, direction: isRTL ? 'rtl' : 'ltr' }">
    ...
    <template v-if="introGlossGroups">
      ...
      >
        ...
        <!-- Gloss line rendered for EVERY run, even empty ones (structural
             ...
             the same baseline. -->
        <span class="block-known gloss-group-known" :lang="knownLang">{{ group.known }}</span>
      </div>
    </template>
        ...
        >
          ...
          <template v-else>
            ...
            <template v-if="isIntroOrDebut">
              ...
              <span v-else-if="block.knownText" class="block-known" :lang="knownLang">{{ block.knownText }}</span>
            </template>
          </template>
        </div>
      </TransitionGroup>
