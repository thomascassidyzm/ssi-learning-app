...
// 2026-07-17 navbar redesign, completed by the same-day Structure
// consolidation: the bar carries the four admin ideas flat — Structure
// (the org tree, ex-Setup) · Invites (doors, promoted out of More) ·
// Users (people) · Stats + Insights (numbers) — and a grouped "More"
// menu carries the rest with icons + one-line descriptions. These tests
// pin the grouping — every admin destination must stay reachable within
// two clicks, and the More trigger must carry the active state when the
// current section lives inside it (so "where am I" survives the collapse).
describe('AdminTopBar — primary tabs + grouped More menu', () => {
  ...
  it('shows the four admin ideas as flat tabs, not all nine', async () => {
    const wrapper = await mountAt('/admin/structure')
    const tabLabels = wrapper.findAll('.tabs > a.tab').map((a) => a.text())
    expect(tabLabels).toEqual(['Structure', 'Invites', 'Users', 'Stats', 'Insights'])
  })
  ...
  it('the More menu contains every collapsed destination, grouped', async () => {
    ...
    expect(itemLabels).toEqual(['Onboarding', 'Methodology'])
  ...
  it('collapsed menu trigger names the CURRENT section, so identity survives the collapse', async () => {
    ...
    expect(items).toEqual(['Structure', 'Invites', 'Users', 'Stats', 'Insights', 'Onboarding', 'Methodology'])
  })
})
