-- rollback_co_teacher_class_page_perf.sql
-- Captured VERBATIM from the live DB immediately before applying
-- 20260807_co_teacher_class_page_perf.sql. Restores the pre-change state exactly.
-- Captured: 2026-08-07T01:29:58.484Z
--
-- Note: my_readable_tag_values() is left in place by this rollback. It is a new
-- object nothing else references once the policies below are restored, and
-- dropping it is not required to undo the change.
...
CREATE POLICY user_tags_select ON public.user_tags
  ...
  USING (
((user_id = (( SELECT auth.uid() AS uid))::text) OR is_god_user() OR (EXISTS ( SELECT 1
   FROM schools s
  ...
  WHERE ((user_tags.tag_value = ('CLASS:'::text || (c.id)::text)) AND ((c.teacher_user_id = (( SELECT auth.uid() AS uid))::text) OR (s2.admin_user_id = (( SELECT auth.uid() AS uid))::text) OR is_class_teacher(c.id))))))
  );
...
((user_id = (( SELECT auth.uid() AS uid))::text) OR is_god_user() OR (EXISTS ( SELECT 1
  ...
  WHERE ((user_tags.tag_value = ('SCHOOL:'::text || (s.id)::text)) AND (s.admin_user_id = (( SELECT auth.uid() AS uid))::text)))) OR (EXISTS ( SELECT 1
   FROM (classes c
     LEFT JOIN schools s2 ON ((s2.id = c.school_id)))
  WHERE ((user_tags.tag_value = ('CLASS:'::text || (c.id)::text)) AND ((c.teacher_user_id = (( SELECT auth.uid() AS uid))::text) OR (s2.admin_user_id = (( SELECT auth.uid() AS uid))::text) OR (is_class_teacher(c.id) AND (user_tags.role_in_context IS DISTINCT FROM 'teacher'::text) AND (user_tags.role_in_context IS DISTINCT FROM 'admin'::text)))))))
  )
  WITH CHECK (
(is_god_user() OR ((user_id = (( SELECT auth.uid() AS uid))::text) AND (role_in_context IS DISTINCT FROM 'teacher'::text) AND (role_in_context IS DISTINCT FROM 'admin'::text)) OR (EXISTS ( SELECT 1
   FROM schools s
  WHERE ((user_tags.tag_value = ('SCHOOL:'::text || (s.id)::text)) AND (s.admin_user_id = (( SELECT auth.uid() AS uid))::text)))) OR (EXISTS ( SELECT 1
   FROM (classes c
     LEFT JOIN schools s2 ON ((s2.id = c.school_id)))
