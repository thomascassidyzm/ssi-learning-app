...
CREATE POLICY user_tags_update ON public.user_tags
  ...
  -- ⚠️ ADDED 2026-08-07, AFTER THE FACT — do not remove.
  --
  -- As first written this CREATE POLICY had a USING clause and NO WITH CHECK.
  -- In Postgres an UPDATE policy with no WITH CHECK reuses its USING clause as
  -- the check, and this USING clause permits `user_id = auth.uid()` for ANY
  -- role_in_context. So the version without this block let ANY authenticated
  -- user run `UPDATE user_tags SET role_in_context='admin' WHERE user_id=me`
  -- and promote themselves to school admin.
  --
  -- That was not theoretical: this file was applied to the live shared DB and
  -- the hole was open there until it was spotted and closed the same day. The
  -- WITH CHECK below is a verbatim carry-over of what user_tags_update had
  -- BEFORE this migration (see 20260806b_co_teacher_manage_ruling.sql, the
  -- founder ruling of 2026-08-06), with only the pointer test swapped for
  -- is_school_admin_of() like everything else here. It is asserted
  -- permanently by supabase/secfix-toolkit/verify_school_admin_tag_parity.cjs.
  --
  -- Lesson for any future policy migration: a DROP + CREATE of an UPDATE
  -- policy must restate the WITH CHECK. Dropping it is silent and it fails
  -- OPEN.
  WITH CHECK (
    public.is_god_user()
    OR (
