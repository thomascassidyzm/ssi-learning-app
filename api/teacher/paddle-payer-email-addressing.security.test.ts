  ...
  const from = (table: string) => {
    ...
    builder.then = (resolve: any) => {
      if (table === 'learner_emails') {
        ...
        return resolve({ data: hit ? [{ learner_id: VICTIM_ADMIN_LEARNER }] : [], error: null })
      }
      if (table === 'learners') {
...
describe('SEC15-04 — the email lookup ignores verification and multiplicity', () => {
  ...
  it('SEC15-04: an unverified learner_emails row is accepted as proof of who paid (vulnerable, characterized)', async () => {
    // The double returns the row without a `verified` field at all; the handler
    // never asks for one, which is the finding.
    await handleSubscriptionEvent(makeSupabase(), attackerSubscription({ kind: 'school_platform' }))
    expect(writes.find((w) => w.table === 'schools')).toBeDefined()
  })

  it.todo('SEC15-04: payer resolution should require learner_emails.verified and reject a multi-learner match')
})
