  ...
  try {
    ...
    if (members.length < effectiveFloor) {
      ...
      const reason = members.length === 0
        ? `No other ${worldNote}${unit} ${scopeNote}exist in this scope yet — a fair comparison needs at least ${effectiveFloor}.`
        : `Only ${members.length} other ${worldNote}${unit} ${scopeNote}exist in this scope — a fair comparison needs at least ${effectiveFloor}.`
      insufficient(reason, members.length)
      return
    }

    const preRows = isGlobalCompare ? null : scopeRows
    ...
    // "school average · all 32 classes on this course" — the average's own
    // denominator, spelled out, so a wider window reading a lower average
    // than a narrower one (fewer classes existed yet, not fewer practising)
