...
describe('SEC0905-X-02 — which vitest config collects which security spec', () => {
  it('the gated api config collects *.security.test.ts', () => {
    ...
    // only entry: `scripts/**/*.test.ts` was added alongside it (scripts/ has
    // no suite of its own), so this matches the glob rather than the whole
    // literal array — same shape as the assertion in
