  ...
  try {
    ...
    const { data: created, error: createError } = await supabase.auth.admin.createUser({
      ...
      user_metadata: {
        ...
        // THE CLASS SEAT (Tom, 2026-09-10, corrected the same night): "who
        // the customer is" decides it. A link-auth mint is only ever a pupil
        // (LINK_AUTH_ELIGIBLE_CODE_TYPES) and it is an INSTITUTION seat —
        // behind a class, funded by the org's own free account, playing the
        // class's course while the org's cover and the class tag are live
        // (api/_utils/classCoverage.ts). That is the product, not a leak,
        // for an eleven-year-old in a Chepstow class and for an adult at a
        // college alike. The stamp records that origin durably on the account
        // — for the census of who is behind an org and who is not — and it
        // gates NOTHING: a class seat adds a real email through
        // api/email/verify.ts like any other account, because that address
        // is its holder's only route back if the device is lost.
