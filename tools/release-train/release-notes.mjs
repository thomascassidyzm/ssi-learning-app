 ...
 * right.
 *
 * SHAPE — FOUNDER RULING (2026-09-12), which supersedes 2026-07-31's "nothing shipped goes
 * unmentioned" and 2026-09-08's "one line on each thing deemed relevant": UP TO 3 most significant
 * features FOR THE USERS, one sentence each, then ONE catch-all section — "Other stuff and bug
 * fixes" — carrying exactly ONE line, by default Tom's own words. Verbatim, on the 2026-09-12
 * ship that carried forty read-more lines: "the release notes for the latest version in Main are
 * crazy. 3 biggest headlines and then + plus squished some bugs and stuff." The earlier rulings
 * for the record — 2026-07-30: "Keep to 3 biggest and then other stuff and bug fixes."; 2026-07-31:
 * overflow features and fixes absorbed one short line each, nothing unmentioned. The per-item
 * record still exists: it lives in the draft-only coverage block, which finalise strips, so a
 * human editing the draft before GO sees what shipped and the learner never does.
 *
...
import {
  extractBullets, findUnrenderable, findOffShape, MAX_HEADLINES, MAX_READMORE, READMORE_LINE,
...
export function buildNotes(cand) {
  ...
  return {
    ...
    // Founder ruling 2026-09-12: the shipped catch-all is ONE line, so this per-item record —
    // overflow features first (still "stuff"), then every fix — is draft-only coverage. It is
    // what a human reads before GO; it never reaches the shipped file.
...
function header({ final, draftDate, shipDate, sha, count }) {
  ...
  L.push('line for everything else that shipped. Deliberately under-claiming: a commit that cannot')
  L.push('be translated confidently into user-facing language is dropped rather than guessed at.')
  L.push('Generated by `tools/release-train/release-notes.mjs`.')
  L.push(HEADER_CLOSE)
  return L.join('\n')
}
...
export function render(cand, notes, { draftDate }) {
  ...
  // Founder ruling 2026-09-12: exactly ONE line below the fold, in Tom's words. The draft carries
  // the same shape the final will, so what a human reads before GO is what learners get.
  if (notes.features.length || notes.otherStuff.length) {
    L.push('## Other stuff and bug fixes')
    L.push('')
    L.push(`- ${READMORE_LINE}`)
    L.push('')
  }
  ...
  if (notes.otherStuff.length) {
    L.push(`**Also shipped — the one line below the fold stands for these (${notes.otherStuff.length}):**`)
...
export function reconcile(draftBody, notes) {
  ...
  // Founder ruling 2026-09-12: at most MAX_HEADLINES headlines, and a line a human typed is the
  // taste pass, so hand headlines take the slots before the machine's own.
  const handFeatures = handOnly("What's new")
  // The draft (pre-2026-07-31 shape) may still carry a "## Fixes" heading rather than "## Other
  // stuff and bug fixes" — read both so an old draft's hand edits are not silently dropped. The
  // default one-liner is machine vocabulary; only a line someone REPLACED it with is a hand edit.
  const otherHand = [...handOnly('Other stuff and bug fixes'), ...handOnly('Fixes')]
    .filter((h) => h !== READMORE_LINE)
  return {
    features: [...new Set([...handFeatures, ...notes.features.map((b) => b.headline)])]
      .slice(0, MAX_HEADLINES),
    // The shipped catch-all: a hand-written line if there is one, else the default at render.
    fixes: otherHand,
    handEdits: [...handFeatures, ...otherHand],
  }
}
...
export function renderFinal({ features, fixes }, { shipDate, sha, count }) {
  ...
  // Founder ruling 2026-09-12: ONE line below the fold — a hand-written one if the draft carries
  // it, else Tom's default. Nothing per-item ships; the record is the draft's coverage block.
  if (features.length || fixes.length) {
    L.push('## Other stuff and bug fixes', '')
    for (const h of (fixes.length ? fixes : [READMORE_LINE])) L.push(`- ${h}`)
    L.push('')
  }
  if (!features.length && !fixes.length) {
    L.push('## What\'s new', '')
...
/**
 * THE SHAPE GATE (Tom's rulings, 2026-09-08 and 2026-09-12): "3 biggest headlines and then + plus
 * squished some bugs and stuff".
 *
 * At most three headlines, one sentence each; exactly ONE line below the fold. The count cap on
 * the catch-all is the 2026-09-12 addition — forty one-sentence lines passed the 2026-09-08 gate.
 * The rule itself lives in notes-bullets.mjs beside the markup predicate — this is only its call site.
...
export function assertShape(body, rel = 'the notes') {
  ...
  throw new Error(
    ...
    `for everything else. Rewrite these bullets:\n${problems.join('\n')}`)
}
