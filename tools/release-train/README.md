...
| `release-notes.mjs` | The notes: **up to 3 features (by user significance, not commit count) then the fixes**, user-facing language, **under-claiming by construction** (untranslatable commits are dropped, never guessed at). Drafts on Thursday, `--finalize` stamps the ship date + promoted sha on Friday, keeping hand edits. |
