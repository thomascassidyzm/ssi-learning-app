 ...
 * generator emits SEED-PHASE reviews (offsets ≥144) that cycles.ts documents
 * as a deliberate gap versus the walk, so extra cycles of exactly that shape
 * are the generator being ahead, never being different.
 */
  ...
  for (const frac of START_FRACTIONS) {
    ...
    try {
      ...
      const seedPhaseKeys = new Set(
        gen.rounds.flatMap((r) => r.cycles.filter((x) => x.id.endsWith('_seedrep')).map(keyOfGen)),
      )
