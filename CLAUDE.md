...
Every capability of the schools dashboard carries its own description, in an HTML comment directly
above the element that IS the capability:
...
<!-- HANDBOOK Remove a teacher from your school
     ...
     roles: school_admin
     place: teachers
     keywords: remove, teacher, leaver, staff
     What it's for. Taking a teacher off your school when they leave. Their own account survives.
     ...
     Worth knowing. Their learners keep every minute they have practised.
     checked: 8f21ba05
-->
<button data-walk="teacher-remove" @click="handleRemoveTeacher(t.user_id, t.name)">Remove</button>
```

Those comments ARE the Handbook at `/schools/handbook` — the page compiles from them, so what a
teacher reads is what the code says. **If you change what a capability does, rewrite its sentence in
the same change.** You are already looking at it; that is the whole point of it living there. Then:
...
|--------|---------|------------|-----|
| `dev` | Rapid integration — Tom's rapid work + ALL `claude/**` web sessions merge here (by hand or through the command surface; nothing auto-merges) | `ssi-learning-app-git-dev-zenjin.vercel.app` (stable Vercel git-branch alias — **there is NO `dev.saysomethingin.app`, it 404s**). **Since 2026-09-14 a push to `dev` builds only when the commit subject carries `[preview]`; a plain push is cancelled in seconds and the alias does not move.** An agent that needs the dev alias to show its change puts `[preview]` on the merge commit's subject, first line only. `staging` and `main` build on every push as before | Tom + Claude |
...
> **Dev URL note.** The only stable non-prod custom domain is `staging.saysomethingin.app`. The `dev` branch has **no** custom domain — use its Vercel git-branch alias `https://ssi-learning-app-git-dev-zenjin.vercel.app` (tracks the latest build of dev, which since 2026-09-14 happens only when the commit subject carries `[preview]`; the alias auto-updates the service worker, so a reload gets fresh code once a build has run). Per-commit hash URLs also work but rotate every push. Dev test cheats: append `?fc=1` (force interjections every boundary), `?stream` (bypass cache play), `?reset=1` (full state wipe).
...
**Current state (updated 2026-08-06 — corrects a stale claim; superseded content below preserved as history):** the learner-data spine (learners, sessions, course_enrollments, lego/seed_progress, daily_contributions, user_tags, class_sessions) has own-row RLS **live since 2026-06-10** — canaried, real-JWT verified. Content tables stay permissive by design. **This section previously said the six org tables (`schools`, `classes`, `groups`, `govt_admins`, `invite_codes`, `entitlement_grants`) remained RLS-off — that is now stale.** Verified live 2026-08-06 (`pg_class.relrowsecurity` + `pg_policies`): RLS is **ON** with real policies on all six — `classes`(6), `schools`(6), `groups`(1), `govt_admins`(4), `invite_codes`(8), `entitlement_grants`(1). The trigger conditions and runbook below describe the pass that closed this gap; they're kept as the record of how it was done and still govern any further policy changes on these tables (canary method, staging soak — see doctrine below), which remains mandatory for any edit, not just the initial enable.
...
1. ~~Enable RLS on the six org tables (five carry dormant pre-authored policies — verify predicates use `auth.uid()::text`, not stale Clerk-era `jwt->>'sub'`, via `pg_get_expr` before trusting them; `groups` + `entitlement_grants` need policies written).~~ **DONE — verified live 2026-08-06:** all six carry `relrowsecurity=true` with active policies (counts above). Any predicate audit against `pg_get_expr` should now be done as a normal review of the live policies, not a pre-enable check.
...
### DO
- Update a schools capability's HANDBOOK comment in the same change that alters what it does
