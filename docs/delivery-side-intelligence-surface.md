...
- **The `app_shell` column.** Absent live when this was written. Applied live on 2026-09-10 by the canary method, see `supabase/migrations/20260904_player_events_app_shell.sql`, so the browser-versus-native split is recorded from that day. Rows written before it carry NULL and read honestly as not recorded; they cannot be back-filled.
