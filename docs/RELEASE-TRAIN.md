...
**The shape is fixed** (founder ruling 2026-07-30, superseding "a handful of bullets"; amended
2026-07-31, verbatim: *"Keep to 3 biggest and then other stuff and bug fixes"*):

```
## What's new                    up to 3 features, the most significant FOR THE USERS
## Other stuff and bug fixes     everything else that shipped: overflow features
                                  (one short line each, same under-claim rule) + the bug fixes
```

So nothing shipped is silent — it is just compact below the fold, instead of being pushed into the
draft-only coverage section for a later hand-promote.
...
take a feature slot from a known surface.

**Ranking is player-first** (founder ruling 2026-07-31, verbatim: *"focus on the player stuff -
most people are not teachers/school leaders"*). When picking the 3 biggest features, player/learner
-facing surfaces are weighted above schools/teacher/leader surfaces — most users are learners, not
teachers or school leaders. Every phrasebook entry carries an `audience` (`player` / `general` /
`schools`), and feature ranking sorts on `significance × AUDIENCE_WEIGHT` (`player: 1.2`,
`general: 1`, `schools: 0.8`) before commit count — so a schools feature never edges out an
equally-significant player feature for one of the 3 headline slots. The weight only breaks
cross-audience ties; it never lets a lower-significance item outrank a higher-significance one.
