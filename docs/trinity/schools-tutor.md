...
**Scoping — server-side vs. client route param:** genuinely server-side for learner-scoped reads (`can_view_learner_data()` confirmed for `course_enrollments`; plausible-but-unverified-in-repo for the other three tables `StudentProgressView` reads). Genuinely client-route-param-only for org tables (`schools`/`groups`/`classes`) — by design, per CLAUDE.md's RLS-tightening doctrine (org tables are intentionally RLS-off until the three named trigger conditions hold). The route-level admin gate itself (`canAccessAdmin`) is client-side with no visible server-side re-check in the four loaders — anyone who can forge past the router guard (e.g. direct API calls, not the SPA) would hit the same RLS/grant layer as any other authenticated user, which for org tables today is permissive by design and for learner-data tables is the `can_view_learner_data()` gate.
