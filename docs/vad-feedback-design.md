...
5. **SCHEMA STEER (founder, mid-build 2026-07-28): `cycle_prosody` logs INTERMEDIATE FEATURES, not just final scalars.** The founder expects to iterate on the prosody metric itself; a stored scalar orphans history every time the metric changes, while stored contours let any future metric be recomputed over all past data. So the event carries compact contour summaries and per-utterance timing features, sized sensibly for telemetry. **The ratified research direction:** standard waveform→prosodic-feature extraction; similarity vs the model voice computed contour/DTW-style, deliberately blind to voice identity and accent; then calibrate variance bands against known-CEFR speakers recorded saying the SAME course phrases — **phrase-matched calibration is SSi's structural advantage** (generic CEFR corpora can't control the utterance; our corpus holds the phrase constant across every learner and every calibration speaker, so the variance bands measure speech, not content). The `cycle_prosody` payload is designed as the raw material for that study. Implementation today: the 20 ms energy envelope (peak-normalized, ≤128-point, 0–100 integer contour + `contourGridMs` timebase) plus full timing features (`speechStartMs`/`speechEndMs`, latency, durations, overlap flags, energy stats). **Pitch contours are not capturable yet** — the live VAD is energy-only by design (build-spec §8, owner decision B); pitch tracking exists in the separate PronunciationOverlay stack and wiring an F0 track into the continuous VAD loop is a phase-2 capture change (CPU/battery cost to validate on phones). When it lands, it extends the same payload block; the energy-domain history stays recomputable regardless.
...
> **RULED 2026-07-28: GO — Option 1 blessed, phase 1 built on this branch.** One `cycle_prosody` event per voiced speaking cycle now logs from `LearningPlayer.vue`'s cycle-complete path (VAD-consented learners only). What it carries **today**: cycle/LEGO/seed identity, the target-voice `audioId` (the join key into `course_audio_envelope`), `response_latency_ms`, `learner_duration_ms`, `duration_delta_ms`, the full learner-side envelope block (`peakCount`, `peakToMeanRatio`, `meanPeakWidthMs`, `sampleCount`, `weight`, and — per the mid-build schema steer, ruling 5 — the intermediate-feature `contour` itself: peak-normalized 0–100 ints, ≤128 points, with `contourGridMs` timebase), `extractorVersion`, capture-quality flags, and energy stats (`peakEnergyDb`, `averageEnergyDb`, `speechStartMs`/`speechEndMs`). Key build finding: the VAD computes the learner envelope **unconditionally** — `stage2_enabled` only gates the adaptation *consumer* — so the corpus is envelope-complete from day one, and the prosody-**match** measure becomes computable **retroactively for every accumulated row** the moment the dashboard-repo envelope pipeline (WP-7b) populates `course_audio_envelope`. No client change is needed to unlock it. Retention: `player_events` has no pruning today; if one is ever added, `cycle_prosody` rows are exempt (this line is the standing marker the ruling asked for).
