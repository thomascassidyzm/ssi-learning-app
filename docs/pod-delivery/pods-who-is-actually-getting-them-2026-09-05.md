...
- **Beuno (beunollyn, deu_for_eng)**: received **3 pod laps ever, all on Aug 7–8** (from the then-live core pod, since retired), and none since — a month of real sessions. His enrollment ratchet reads `completed_pod_rounds = 0` despite those 3 completed laps, so even the delivery record failed to record delivery.
