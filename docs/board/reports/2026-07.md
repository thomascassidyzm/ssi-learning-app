...
The whole schools "region tier" is now built and hardened. The honest number today is **{{metric:schools.total}}** — every school currently in the database is our own soak-test, E2E, or partner-demo data (Gwynedd/Bangor pilots-in-progress, the IME demo programme, Ireland's Gaelscoileanna demo), not yet a live paying or claimed pilot school; the growth story this month is the machinery being ready, not a school count. First real school lands when the Gwynedd/Bangor and IME pipelines convert an invite into a claimed, paying school. The shape: a group leader (a council, a trust, a partner like IME) gets one link. Clicking it makes them the owner of their group. From there they mint one-link invites for schools; a school admin clicks, verifies their email once, and lands on a working dashboard — no forms, no second sign-in, no setup wizard standing in the way. Schools a leader creates show honestly as "awaiting admin" until someone claims them. Teachers press play and teach the whole class from one screen.
...
The same rule now runs the board report itself: in the last 30 days, **{{metric:learners.active_30d}}** learners were active on the platform, practising a combined **{{metric:minutes.total_30d}}** minutes.
