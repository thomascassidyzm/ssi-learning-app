...
**Re-checked after the fact, on the code that is actually on dev now.** This writeup was interrupted by a server restart, so the fence was run again from scratch against the current head of `dev` rather than trusted from memory: the whole player suite is green — 52 tests, including all six A-22 ones and the tap-case carve-out. Nothing has drifted since it landed.
...
- **Another A-22 job got there first.** A parallel remediation nulled 17,489 dead pointers earlier today, with the foreign key switched on so they cannot come back, and a language-aware relink of the ~6,000 recoverable clips is running as I write. So this reads the estate *after* that clean-up, not independently of it. What I can say is that there is nothing left for a regeneration run to chase.
- **What a nulled pointer does to a session, in the code as it stands.** An emptied clip can no longer point at a file that isn't there, which is what killed A-22. For the opening item of a LEGO the player falls back to the known-language clip and carries on; anywhere else, an item missing any of its three clips is dropped while the session is being built and simply never reaches the player. So the failure has moved from fatal to quiet — which is exactly why the skip-counter above matters.
...
**The count of courses is not 132.** The courses table holds **143 rows — 16 released, 63 beta, 64 draft**, counted twice by two people with the privileged key, hours apart, and the same both times. The 88 in this morning's chopped-clip census was an undercount: the public database key cannot see 55 of the courses, almost all drafts, so anything counted with it is short. **132 remains unaccounted for and I am calling that a gap rather than closing it.** Two independent sweeps went looking: every status filter, every catalogue query the live site actually runs, language-pair counts both ordered and unordered, courses with content, courses with enrolments, courses with played sessions, and a text search for the number across both repositories' code, docs and history. The nearest real numbers are 143 rows in total, 100 with content generated, 93 with a real learner enrolled, 80 in the live catalogue, 75 ever played. None of them is 132 and no combination of the fields produces it. If 132 came from somewhere outside these two systems — the old app, a spreadsheet, a marketing page — say where and I will reconcile against it; otherwise the honest answer is that the estate is 143 and the earlier 88 was a filtered view of it. Worth knowing that "released" as the database means it covers only 16 courses, while real learners are active on 53 — including many marked beta. If beta courses have live learners, the release gate needs to apply to them too.
