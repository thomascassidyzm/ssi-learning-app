...
- **TUTOR LANE — `platform_status` written from a browser-supplied `teacher_id`, no guard at all (A-123 residue, PRE-EXISTING)** — `handleTutorPlatformSubscription` (`api/teacher/paddle-webhook.ts:976`) takes `customData.teacher_id` straight from the checkout payload, looks up nothing to prove the payer owns that teacher row, and `.update()`s `platform_status` + `platform_expires_at` on it — no binding check, no rung-1 short-circuit, none of the A-123 ladder. Same shape of hole A-123 closed on the school/org and learner lanes; A-123 did **not** touch this function and does not make it worse, but its write-up's "who paid is now settled before the money moves" does not cover this lane. Unlike the learner lane this one can *remove* access: a cancellation event addressed at someone else's `teacher_id` writes `platform_status:'cancelled'` onto their row. **Live exposure today: 7 teacher rows, 5 trial, 0 paying — so nothing is at risk right now, which is why it was deferred rather than fixed in A-123.** Fix shape: bind the tutor lane to the same `billingBinding`/`billingIntent` ladder the other two lanes now use. Founder ruling 2026-08-16 (A-123 promotion): file and track, do not block the release train. — untouched — worker — M
- **A-123 residue: a first purchase refused when the payer's email maps to more than one learner** — `resolvePayerAuthUid` refuses when the `learner_emails` row is unverified **or** multi-match; multiple accounts per person is *intentional* on this estate (testers), so multi-match is an expected state. No access is lost and nothing is written — a legitimate first purchase simply fails to attribute. Founder ruling 2026-08-16: rides to staging and production as documented known behaviour, NOT fixed now; if it fires on a real buyer it is Tom's call with that buyer's situation in hand. → `docs/a123-landing-blocker-2026-08-16.md` — known-accepted — founder — S
- **A-123 residue: an overlapping plan upgrade bought before the old one lapses needs manual intervention** — a learner on a still-active plan who buys a higher one (Premium → Family) before the old lapses has the row write blocked by `wouldDowngradePlan`'s live-plan guard. They keep their existing plan — nothing is removed — but the upgrade they paid for does not apply until someone intervenes. Untested case. Founder ruling 2026-08-16: rides along as documented known behaviour, NOT fixed now; escalate to Tom if a real buyer hits it. → `docs/a123-landing-blocker-2026-08-16.md` — known-accepted — founder — S
