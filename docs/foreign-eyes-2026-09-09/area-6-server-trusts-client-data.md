...
`api/school/remove-staff.ts` and the class-removal paths revoke membership by setting `removed_at` (`api/school/remove-staff.ts:113–118`). `user_tags_update`'s self-branch lets the subject of that removal write their own row back:
...
The premise holds for an OTP signup. It does **not** hold for a possession-redeem account, and the repo says so in the very file that creates one — `api/auth/possession-redeem.ts:434–437`:

```text
434:     const { data: created, error: createError } = await supabase.auth.admin.createUser({
435:       email: normalizedEmail,
436:       email_confirm: false,
```

`normalizedEmail` is the address the caller typed into the redemption form. `api/code/redeem.ts:344–346` describes the resulting account in as many words:

```text
344:     // Possession-onboarded accounts (api/auth/possession-redeem.ts) never
345:     // prove mailbox receipt — that endpoint mints a session without ever
346:     // emailing anyone.
```
...
2. Pick a target address that has **not yet signed up** — which is precisely the population an email allowlist grant exists to serve, and which `api/access/grant-emails.ts:161–163` calls "waiting users".
3. `POST /api/auth/possession-redeem` with that code and the target's address. The already-registered refusal (`api/auth/possession-redeem.ts:452–468`) does not fire, because the target has no account. A session comes back.
...
`api/_utils/actAsGuard.ts:20–27`:
...
...
26: export function isViewAsRequest(req: VercelRequest): boolean {
27:   return req.headers['x-ssi-view-as'] === '1'
28: }
```
...
- **`api/family/*` (7 routes), `api/try-link/*`, `api/invite/create.ts`, `api/onboarding/provision.ts`, `api/teacher/commissions.ts`, `api/teacher/payout-recipient.ts` and `api/cron/teacher-payouts.ts`** got a grep-level pass for the shape and no closer reading. The payout trio is the gap I would fill first: it is the only money-*out* path in the estate and I did not trace what its amounts are derived from.
