...
| B | No-chatter soak 3+ min; manual refresh; honest Updated stamp | **FAIL → FIXED** — soak PASS (zero background API calls in 3 min idle, 0 even exempt); manual refresh affordance present and refetches (1 request on click). Updated stamp was BLANK on node home until a manual refresh → fixed (see below), re-walked green on the promoted build `9bb3d26` ("Updated 06:14" on cold load; fw-11) |
...
---
## Fix re-walk (staging `9bb3d26`)

The two fixes were promoted dev→staging (standing authorization) and re-walked on the deployed build:

- **5b Updated stamp:** PASS — cold navigation to the group node home shows "Updated 06:14" with no manual refresh (`founder-walk-stampcheck.mjs`, fw-11-updated-stamp.png).
- **Greeting:** PASS — the straight-in landing now reads "Welcome back." with the "Updated 06:15" stamp present; still authenticated, zero interstitials, 8.3s (fw-10-straight-in.png re-shot on the new build).

*First full-walk result on `a23c989`: 24/26 (5b stamp — fixed above; 7a — walk-tooling selector fault, re-walked PASS). Final state: 8/8 checklist items green.*
