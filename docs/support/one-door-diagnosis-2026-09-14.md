...
| Handbook ask | the schools Handbook's ask box | school staff | handbook_questions, via api/handbook-questions, which is on branch cs/386 and not on dev | auth uid only | nothing polls it. One probe row exists. Found by job #676's census, missed by the code read of this tree |
...
Job #676's census, read after this pass, agrees on every door above and adds the Handbook ask table. No real message from a learner or school is sitting unread in the one watched table. The unread real reports are Aran's two in tester_feedback, which the tester widget stopped writing to only this morning, and nba4191's four from April and May.
...
3. A service-role view, support_inbox, unions bug_reports with inbound support_messages and, for history, tester_feedback, content_feedback and handbook_questions, so "are there any messages" is one query.
