...
| **Listening — the pod** | The dialogue scenes you listen through | Spanish: 231 sentences across 22 scenes = **571 clips, 45 minutes** | **~10 MB** |
| **Listening — Layer 1** | The comprehensible-input sandwiches built from seeds you've already drained | Spanish: **1,334 clips, 80 minutes** | **~18 MB** |
| **Commentary** | The spoken encouragements, instructions and the welcome | A small fixed set per course | Under a megabyte |

So **all the listening together is about 1,905 clips, two hours, ~28.7 MB** on Spanish — a single-digit percentage of the course, and the part you can simply loop through when nothing new is reachable.
...
The one thing genuinely wrong was where listening sat, and that is now fixed. The cost is **~28.7 MB fetched earlier than before** on a Spanish-sized course. That is a real number on someone's mobile data and it is your call, not mine — but for what it buys, two hours of material a learner can loop through the moment new content stops arriving, it looks cheap.
