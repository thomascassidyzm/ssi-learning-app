...
> **RECONCILED ONTO `dev` 2026-09-06 (job #912).** This audit's tests were merged into `dev` by the
> #900 sweep, went red on eight characterizations, and were backed out rather than guessed at. They
> are now back, reconciled. Two of this audit's own findings had been FIXED on `dev` in the meantime,
> so their characterizations were rewritten as secure-assertions of the shipped fix — the tests were
> updated, never the code:
>
> | Finding | Status on `dev` | Fixed by |
> |---|---|---|
> | **A-02** account pre-hijacking via shell adoption (CRITICAL) | **FIXED** — adoption is bound to the invite that created the shell, via a service-role-only `app_metadata` claim (`api/_utils/shellClaim.ts`) | job #557 |
> | **A-01** staff-signin-link containment tags-only (HIGH) | **FIXED** — containment runs on `schoolReachOf`, which unions `schools.admin_user_id`, SCHOOL: tags of any role, and CLASS: tags resolved to their school | job #565 |
> | **A-03** `staff_access_codes` has no migration (MEDIUM) | **PARTLY CLOSED** — `supabase/schema.sql` was regenerated (`a7384811`) and now records the table with RLS on and a `service_role`-only grant. **Residual, still open: no migration file creates it**, so the table cannot be built from the repo. |
>
> The remaining two reds were machinery, not findings: the pinned `*.security.test.ts` roster (grown
> by the six `sec0905` files) and a literal-string assertion on `vitest.api.config.ts`'s `include`,
> which `dev` had widened to add `scripts/**/*.test.ts`. Both now assert the glob rather than the
> whole array.
>
> Also landed in the same pass: `cs/595`'s school-authority agreement tests, hand-merged against the
> `schoolReachOf` suite `dev` grew — including a genuinely new finding of its own, that
> `ensureSchoolAdminTag` treated `23505` as proof of a grant when a REVOKED tag holds the unique key,
> so re-granting admin to a previously-removed person silently did nothing. That now fails loudly.
