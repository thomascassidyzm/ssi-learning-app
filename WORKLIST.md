...
- [~] @claude-local 06-13 **Metrics foundation (workstream A) — rollup write (Option 2, decided via BSC).** Fork resolved: persist a bounded recent-samples series on the Layer-1 row (small additive A3 column) so B1/B4 read it cheaply — chosen over Layer-0 read-time reconstruction (matches the plan's chain; one-row reads; bounded cost). Building: migration file + extend `LegoMetricsStore` + rollup append in the player + B4 load-and-assess. A1 (phase-pill events) live; A3 columns applied (`mean_latency_ms`/`mean_exec_score`/`skip_back_count`/`skip_forward_count`/`next_due_at`/`device_class_mix`); existing write path = `core/persistence/LegoMetricsStore.upsertMany` (called from `LearningPlayer.vue`, currently writes only the *old* mastery columns). **The gap:** B1 curvature + B4 difficulty need a time-ordered *series* per (learner,unit); A3 stores a single rolling *scalar* — no history. **Decide the series source:** (1) reconstruct from Layer-0 events at read time (no new schema; read-time cost), or (2) persist a bounded recent-samples array on the Layer-1 row (small A3 schema add — *recommended*: the plan's chain has sensors reading Layer 1, B4 `depends on A3`, and reads stay cheap). The plan flags A3 schema as **single-careful-owner** → metrics-owner / Tom's call, not a cross-stream guess. After the decision: extend `LegoMetricsStore` + its caller to write the chosen shape, then point B4 at it. → `metrics-implementation-plan.md` §1A/§2, `localDifficulty.ts` header
