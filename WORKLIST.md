...
- [~] @claude-local 07-03 **MONEY: guest belt-picker bypasses the Yellow-belt trial wall (found 07-02 by test fleet, VERIFIED PRE-EXISTING ON PROD):** the "Next belt" stepper correctly stops at the free-preview boundary, but the belt-PICKER modal (progress-pill tap) lets a signed-out guest jump to any belt on a premium course and PLAY there — zero entitlement checks fire (repro scripts + screenshots in the 07-02 session scratchpad probes dir; pw-*.mjs / v1-*.mjs). Fix = apply the same canAccessSeed gate to picker jumps; server-side strict entitlement (e95bb611 scaffold) is the belt-and-braces follow-up. Also two nits from the same fleet: welcome-audio uuid double-fetched on first load; demo:selectCourse handler should enrich its stub course object (undefined target_lang throws toUpperCase in useI18n via PlayerRestingState).
