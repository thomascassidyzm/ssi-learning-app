...
- [~] @claude-local 06-14 **Teacher-tag write endpoint (service-role)** mirroring `/api/teacher/create-class-join-code` (live RLS forbids non-god teacher tags) — *gates co-teaching: lets a class gain/lose/hand-over a teacher.* → `tutor-insights.md` §7.7
...
- [!] **Metrics M1+** — A1 + A3 + the rollup series write (`f9929aaf`) all landed; the only remaining M0 gate is **applying `20260613_metrics_lego_latency_series.sql`** (Tom). After apply, M1 (curvature/difficulty on live data; teacher needs-attention + comparison) is unblocked.
