...
- [!] **RLS rebase: ownership→membership — ASSESSED 06-14, deliberately deferred.** Risk read (4-agent): low risk to what matters (core learner play = own-row policies, money = service-role webhooks — both untouched; blast radius = the pilot-only schools dashboard; monotonic widening; 0 co-teachers today = no-op for every current reader; one-statement revert). BUT the naive "swap one clause" has two real foot-guns the doc now corrects (§5): **`classes`/`schools` RLS is actually OFF — do NOT enable it** (CLAUDE.md's first-paying-school detonation), and the learner-data predicate is inside the **`can_view_learner_data()` function** (gates 6 surfaces), not the per-table policies — miss it and co-teaching half-works (empty gradebook). **No live consumer yet** (no co-teacher exists; no add-teacher UI), so this rides CLAUDE.md's gated/staged schools-RLS tightening rather than a now-apply. Exact edit list + pre-conditions in `class-first-class-citizen.md` §5.
