...
- [~] @web 06-14 **Synthetic telemetry for demo learners (make the curves demoable)** — extend `generate-demo-suite.cjs` to populate `learner_lego_metrics` (incl. the `recent_latency_samples` series + Layer-1 scalars) and a bounded set of `player_events` for the `is_demo` learners, with calibrated struggling/easing/steady curve shapes, so the difficulty-turns board + curvature engine light up on *named* demo learners on the real path (not just `?demo`). Requires migrations 20260613 (a3 + latency series) + 20260614 applied. → `tutor-insights.md` §3
